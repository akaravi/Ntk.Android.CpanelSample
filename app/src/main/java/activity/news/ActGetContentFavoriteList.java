...
public class ActGetContentFavoriteList extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.act_news_get_content_favorite_list);
        ButterKnife.bind(this);
        initialize();
    }

    private void initialize() {
        lblLayout.setText("NewsContentFavoriteList");
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setDisplayShowHomeEnabled(true);
        getSupportActionBar().setTitle("NewsContentFavoriteList");
    ...
    private void getData() {
        NewsContentFavoriteListRequest request = new NewsContentFavoriteListRequest();
        ...
        RetrofitManager manager = new RetrofitManager(ActGetContentFavoriteList.this);
        INews iNews = manager.getRetrofit(configStaticValue.ApiBaseUrl).create(INews.class);
        Map<String, String> headers = new HashMap<>();
        headers = configRestHeader.GetHeaders(this);
        headers.put("PackageName", EasyPreference.with(this).getString("packageName",""));

        Observable<NewsContentFavoriteListResponse> call = iNews.GetContentFavoriteList(headers, request);
