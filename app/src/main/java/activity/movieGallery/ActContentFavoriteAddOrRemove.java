...
public class ActContentFavoriteAddOrRemove extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.act_movie_gallery_content_favorite_add_or_remove);
        ButterKnife.bind(this);
        initialize();
    }

    private void initialize() {
        lblLayout.setText("MovieGalleryContentFavoriteAdd/MovieGalleryContentFavoriteRemove");
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setDisplayShowHomeEnabled(true);
        getSupportActionBar().setTitle("MovieGalleryContentFavoriteAddOrRemove");
    }
    ...
    private void addFavorite() {
        MovieGalleryContentFavoriteAddRequest request = new MovieGalleryContentFavoriteAddRequest();
        ...
        call.observeOn(AndroidSchedulers.mainThread())
                ...
                .subscribe(new Observer<MovieGalleryContentFavoriteAddResponse>() {
                    @Override
                    public void onSubscribe(Disposable d) {
                    }

                    @Override
                    public void onNext(MovieGalleryContentFavoriteAddResponse response) {
    ...
    private void removeFavorite() {
        MovieGalleryContentFavoriteRemoveRequest request = new MovieGalleryContentFavoriteRemoveRequest();
        if (!txtIdAddOrRemove.getText().toString().matches("")) {
            if (txtIdAddOrRemove.getInputType() != InputType.TYPE_CLASS_NUMBER) {
                txtIdAddOrRemove.setError("InValid Info !!");
        ...
        call.observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
                .subscribe(new Observer<MovieGalleryContentFavoriteRemoveResponse>() {
                    @Override
                    public void onSubscribe(Disposable d) {
                    }

                    @Override
                    public void onNext(MovieGalleryContentFavoriteRemoveResponse response) {
                        JsonDialog cdd = new JsonDialog(ActContentFavoriteAddOrRemove.this, response);
                        cdd.setCanceledOnTouchOutside(false);
                        cdd.show();
                    }
